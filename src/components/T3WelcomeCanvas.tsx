  ...
  function updateBoxAnimations(boxes: THREE.Mesh[]) {
    ...
    if (currentTime - lastUpdateTimeRef.current > updateInterval) {
      ...
      for (let i = 0; i < boxes.length; i++) {
        ...
        animation.targetPosition[randomAxis] =
          positionRange * (Math.random() * 2 - 1);
    ...
    for (let i = 0; i < boxes.length; i++) {
      ...
      if (animation.animationProgress < 1) {
        ...
        box.position.x =
          ...
          (animation.targetPosition.x - animation.startPosition.x) *
            easedProgress;
        box.position.y =
          animation.startPosition.y +
          (animation.targetPosition.y - animation.startPosition.y) *
            easedProgress;
        box.position.z =
          animation.startPosition.z +
          (animation.targetPosition.z - animation.startPosition.z) *
            easedProgress;
      }
    }
  }
